...
@Component({
  ...
  template: `
    <form *ngIf="donut" class="donut-form" (ngSubmit)="handleSubmit(form)" #form="ngForm">
      <label>
        <span>Name</span>
        <input
          type="text"
      ...
      <button *ngIf="!isEdit" [disabled]="form.invalid" type="submit" class="btn btn--green">
        Create
      </button>
      <button
        *ngIf="isEdit"
      ...
      <button *ngIf="form.touched || isEdit" type="button" class="btn btn--grey" (click)="form.resetForm()">
        Reset Form
      </button>
